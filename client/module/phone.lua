...
RegisterNetEvent('A7-voice:syncCallData', function(callTable, channel)
	callData = callTable
...
RegisterNetEvent('A7-voice:addPlayerToCall', function(plySource)
	toggleVoice(plySource, true, 'call')
	callData[plySource] = true
end)

RegisterNetEvent('A7-voice:removePlayerFromCall', function(plySource)
...
function setCallChannel(channel)
	...
	TriggerServerEvent('A7-voice:setPlayerCall', channel)
	callChannel = channel
	sendUIMessage({
		callInfo = channel
	})
...
RegisterNetEvent('A7-voice:clSetPlayerCall', function(_callChannel)
	if GetConvarInt('voice_enableCalls', 1) ~= 1 then return end
