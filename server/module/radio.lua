...
function addPlayerToRadio(source, radioChannel)
	if not canJoinChannel(source, radioChannel) then
		...
		return TriggerClientEvent('A7-voice:removePlayerFromRadio', source, source)
	end
	...
	for player, _ in pairs(radioData[radioChannel]) do
		TriggerClientEvent('A7-voice:addPlayerToRadio', player, source, plyName)
	end
	voiceData[source] = voiceData[source] or defaultTable(source)
	voiceData[source].radio = radioChannel
	radioData[radioChannel][source] = false
	TriggerClientEvent('A7-voice:syncRadioData', source, radioData[radioChannel], GetConvarInt("voice_syncPlayerNames", 0) == 1 and plyName)
end
...
function removePlayerFromRadio(source, radioChannel)
	...
	radioData[radioChannel] = radioData[radioChannel] or {}
	for player, _ in pairs(radioData[radioChannel]) do
		TriggerClientEvent('A7-voice:removePlayerFromRadio', player, source)
	end
...
function setPlayerRadio(source, _radioChannel)
	...
	if isResource then
		...
		-- changed
		TriggerClientEvent('A7-voice:clSetPlayerRadio', source, radioChannel)
	end
...
RegisterNetEvent('A7-voice:setPlayerRadio', function(radioChannel)
	...
	if radioTbl then
		...
		for player, _ in pairs(radioTbl) do
			if player ~= source then
				TriggerClientEvent('A7-voice:setTalkingOnRadio', player, source, talking)
