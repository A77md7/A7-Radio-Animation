...
function removePlayerFromCall(source, callChannel)
    ...
    for player, _ in pairs(callData[callChannel]) do
        TriggerClientEvent('A7-voice:removePlayerFromCall', player, source)
    end
...
function addPlayerToCall(source, callChannel)
    ...
    for player, _ in pairs(callData[callChannel]) do
        ...
        if player ~= source then
            TriggerClientEvent('A7-voice:addPlayerToCall', player, source)
        end
    end
    callData[callChannel][source] = true
    voiceData[source] = voiceData[source] or defaultTable(source)
    voiceData[source].call = callChannel
    TriggerClientEvent('A7-voice:syncCallData', source, callData[callChannel])
end
...
function setPlayerCall(source, _callChannel)
	...
	if isResource then
		...
		-- changed
		TriggerClientEvent('A7-voice:clSetPlayerCall', source, callChannel)
	end
...
RegisterNetEvent('A7-voice:setPlayerCall', function(callChannel)
